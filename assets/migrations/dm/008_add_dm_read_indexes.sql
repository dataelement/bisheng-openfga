...
CREATE INDEX IF NOT EXISTS idx_tuple_read_object ON tuple (store, object_type, object_id, ulid);

-- Read by user + relation: WHERE store, _user, relation ORDER BY ulid LIMIT N.
CREATE INDEX IF NOT EXISTS idx_tuple_read_user ON tuple (store, _user, relation, ulid);

-- Read by user only: WHERE store, _user ORDER BY ulid LIMIT N.
CREATE INDEX IF NOT EXISTS idx_tuple_read_user_only ON tuple (store, _user, ulid);

-- +goose Down
DROP INDEX IF EXISTS idx_tuple_read_object;
DROP INDEX IF EXISTS idx_tuple_read_user;
DROP INDEX IF EXISTS idx_tuple_read_user_only;
